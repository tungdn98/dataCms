...
public class SaleOpportunityResource {
    ...
    @PostMapping("/sale-opportunities/batch")
    public ResponseEntity<Integer> saveBatchSaleOrders(@RequestBody List<SaleOpportunity> saleOppos) {
        log.debug("REST request to save a list of saleOppos : {}", saleOppos);
        for (SaleOpportunity saleOpportunity : saleOppos) {
            if (saleOpportunity.getId() != null) {
                throw new BadRequestAlertException("A new company cannot already have an ID", ENTITY_NAME, "idexists");
            }
        }
        Integer savedCount = saleOpportunityRepository.saveAll(saleOppos).size();
        return ResponseEntity
            .ok()
