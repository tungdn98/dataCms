...
import { ActivityTypeImport } from 'app/shared/model/activity-type.model';
...
export const ActivityType = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const activityTypeList = useAppSelector(state => state.activityType.entities);
  const loading = useAppSelector(state => state.activityType.loading);
  const totalItems = useAppSelector(state => state.activityType.totalItems);
  ...
  return (
    <div>
      <h2 id="activity-type-heading" data-cy="ActivityTypeHeading">
        Activity Types
        <div className="d-flex justify-content-end">
          ...
          <Link to="/activity-type/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
      ...
      <div className="table-responsive">
        {activityTypeList && activityTypeList.length > 0 ? (
            ...
            <tbody>
              {activityTypeList.map((activityType, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/activity-type/${activityType.id}`} color="link" size="sm">
                      {activityType.id}
                    </Button>
                  </td>
                  <td>{activityType.activityTypeId}</td>
                  <td>{activityType.activityType}</td>
                  <td>{activityType.textStr}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/activity-type/${activityType.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button
                        tag={Link}
                        to={`/activity-type/${activityType.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/activity-type/${activityType.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Activity Types found</div>
        )}
      </div>
      {totalItems ? (
        <div className={activityTypeList && activityTypeList.length > 0 ? '' : 'd-none'}>
      ...
      <Dialog
        header="Import dữ liệu activity types"
        visible={visibleImportDialog}
        style={{ width: '70vw' }}
        onHide={() => setVisibleImportDialog(false)}
        breakpoints={{ '960px': '75vw', '641px': '100vw' }}
      >
        <ImportComponent model={ActivityTypeImport} endpoint="/api/activity-types/batch" />
      </Dialog>
