...
import { ICurrency, CurrencyImport } from 'app/shared/model/currency.model';
...
export const Currency = (props: RouteComponentProps<{ url: string }>) => {
  ...
  return (
    <div>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('currencyNum')}>
                  Currency Num <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('conversionRate')}>
                  Conversion Rate <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {currencyList.map((currency, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{currency.currencyNum}</td>
                  <td>{currency.currencyCode}</td>
                  <td>{currency.currencyName}</td>
                  <td>{currency.currencyExchangeRateId}</td>
      ...
      <Dialog
        header="Import dữ liệu currency"
        visible={visibleImportDialog}
        style={{ width: '70vw' }}
        onHide={() => setVisibleImportDialog(false)}
        breakpoints={{ '960px': '75vw', '641px': '100vw' }}
      >
        <ImportComponent model={CurrencyImport} endpoint="/api/currencies/batch" />
      </Dialog>
